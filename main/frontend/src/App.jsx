...
import { LoginPage, SignupPage, VerifyOTPPage, ForgotPasswordPage, ResetPasswordPage } from './pages/auth';
...
function ProtectedRoute({ children, allowedRoles = [] }) {
  ...
  // Restrict admin/organiser modules to a specific email
  const adminEmail = 'ayushdonga111@gmail.com';
  const requiresAdminEmail = normalizedAllowedRoles.includes('ADMIN') || normalizedAllowedRoles.includes('ORGANISER');

  if (normalizedAllowedRoles.length > 0 && !normalizedAllowedRoles.includes(userRole)) {
    return <Navigate to="/dashboard" replace />;
  }

  if (requiresAdminEmail && user?.email?.toLowerCase() !== adminEmail.toLowerCase()) {
    return <Navigate to="/dashboard" replace />;
  }
  ...
  return (
    <BrowserRouter future={{ v7_startTransition: true, v7_relativeSplatPath: true }}>
      ...
      <Routes>
        ...
        {/* Organiser routes - high-level only */}
        ...
        {/* Provider routes - operational dashboard */}
